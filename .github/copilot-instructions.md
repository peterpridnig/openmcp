...
`scr_python/webcam_grab.py` (stdlib-only Python CLI) captures a still frame from a Linux V4L2 webcam by shelling out to `ffmpeg`. `mcp/openpaw_mcp_server.py` is a general-purpose MCP server (official SDK, `mcp>=2`) that reuses the CLI's helpers and exposes `list_webcams()` and `capture_still()` (inline image content) over stdio; more tool groups are expected to follow.
...
- A webcam (real hardware) is required for actual captures; in headless environments only `python3 -m py_compile scr_python/webcam_grab.py mcp/openpaw_mcp_server.py` and `python3 scr_python/webcam_grab.py -h` can be validated. Captures default into `tmp/` (gitignored).
